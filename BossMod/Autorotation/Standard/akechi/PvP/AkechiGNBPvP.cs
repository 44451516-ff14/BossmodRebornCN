...
public sealed class AkechiGNBPvP(RotationModuleManager manager, Actor player) : AkechiTools<AID, TraitID>(manager, player)
{
    ...
    {
        var res = new RotationModuleDefinition("Akechi GNB (PvP)", "PvP 循环模块", "PvP", "Akechi", RotationModuleQuality.Basic, BitMask.Build((int)Class.GNB), 100, 30, PvP: PvPCompatibility.PvPOnly);
