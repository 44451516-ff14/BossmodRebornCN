...
public sealed class AkechiWHMPvP(RotationModuleManager manager, Actor player) : AkechiTools<AID, TraitID>(manager, player)
{
    ...
    {
        var res = new RotationModuleDefinition("Akechi WHM (PvP)", "PvP 循环模块", "PvP", "Akechi", RotationModuleQuality.Basic, BitMask.Build((int)Class.WHM), 100, 30, PvP: PvPCompatibility.PvPOnly);
        res.Define(Track.Targeting).As<TargetingStrategy>("目标选择", "", 300)
            .AddOption(TargetingStrategy.Auto, "自动选择最佳目标")
            .AddOption(TargetingStrategy.FocusTargetsTarget, "自动选择当前焦点目标的目标；若无焦点目标或焦点目标为敌对单位，则自动选择最佳目标")
        ...
        res.Define(Track.RoleActions).As<RoleActionStrategy>("职能技能", "", 300)
            ...
            .AddOption(RoleActionStrategy.Haelan, "自身血量低于 70% 时使用 Haelan")
            .AddOption(RoleActionStrategy.Stoneskin2, "自身血量低于 80% 或被至少 2 个敌人选中时使用 Stoneskin II")
            .AddOption(RoleActionStrategy.Diabrosis, "有技能时对最佳目标使用 Diabrosis");
